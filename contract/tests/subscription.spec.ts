...
import { airdrop, sleep, TEST_ID } from "./util/setup";
...
describe("Subscription", () => {
  ...
  beforeAll(async () => {
    ...
    await testMint.transfer(null, user.publicKey, amount.muln(4).toNumber());
  ...
  describe("charge_subscription_payment", () => {
      ...
      await expectThrowError(
        () =>
          program.program.methods
            .chargeSubscriptionPayment()
            .accounts({
              sender: payment_delegate.publicKey,
              subscription: subscriptionAccount,
              user: userAccount,
              state,
              service: serviceAccount,
    ...
    it("fail - untimely payment.", async () => {
      const [userAccount] = program.findUserAddress(user.publicKey);
      const [serviceAccount] = program.findServiceAddress(id);
      const [subscriptionAccount] = program.findSubscriptionAddress(
        user.publicKey,
      ...
      await program.updateServiceSubscriptionPeriod(id, new BN(0));
    });
